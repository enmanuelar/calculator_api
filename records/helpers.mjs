...
export const getRecordsByUserId = async (userId, page = 0, limit = 10) => {
  ...
  const query = `SELECT user_id as userId, user_balance as balance, operation_result as result, date, amount as cost, type as operationType  FROM records r JOIN operations o on o.id = r.operation_id WHERE user_id="${userId}" LIMIT ${limit} OFFSET ${offset}`;
  const response = await invokeDB(query);
  return JSON.parse(response.body).data;
};

export const getTotalRecordsCountByUserId = async (userId) => {
  const query = `SELECT COUNT(id) AS total_count FROM records WHERE user_id="${userId}"`;
  const response = await invokeDB(query);
  return JSON.parse(response.body).data;
};
